...
async function readPortal() {
  ...
  const [terms, courses, runs, sessions, students, teachers, classrooms, enrollments, invoices, payments, attendance, resourceBookings, teacherBookings] = await Promise.all([
    ...
    rows(`SELECT class_attendance.*, class_student_bookings.class_session_id, class_student_bookings.allocated_fee, students.name AS student_name, class_sessions.topic, class_sessions.starts_at, class_sessions.ends_at, class_runs.name AS run_name, course_catalogs.title AS course_title
