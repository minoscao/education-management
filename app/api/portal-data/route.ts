...
async function readPortal() {
  ...
  const [terms, courses, runs, sessions, students, teachers, campuses, classrooms, enrollments, invoices, payments, attendance, resourceBookings, teacherBookings, businessHoursSetting] = await Promise.all([
    ...
    rows(`SELECT class_runs.*, course_catalogs.title AS course_title, course_catalogs.subject, course_catalogs.display_color AS course_color, academic_terms.name AS term_name, COUNT(DISTINCT class_sessions.id) AS session_count, COUNT(DISTINCT class_enrollments.id) AS student_count
