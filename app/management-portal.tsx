...
function CourseCatalogueDrawer({ detail, data, busy, run, close, closeAll, openDetail }: { detail: Exclude<Detail, null>; data: PortalData; busy: boolean; run: (action: string, values?: Row) => Promise<void>; close: () => void; closeAll: () => void; openDetail: (detail: Exclude<Detail, null>) => void }) {
  ...
  return <div className="sheet-backdrop" role="presentation" onMouseDown={closeAll}><aside className="detail-sheet detail-course" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><header className="entity-header"><div className="entity-heading"><div className="entity-icon"><BookOpen size={22} /></div><div><span className="sheet-eyebrow">Course product</span><h2>{get(course, "title")}</h2><p>{get(course, "subject")} · {get(course, "level")} · SKU {get(course, "code")}</p></div></div><div className="entity-header-actions"><button className="quiet-button" type="button" onClick={() => setTab("settings")}>Edit course</button></div></header><div className="entity-workspace detail-workspace"><main className="entity-main"><CourseCatalogueContent tab={tab} course={course} intakes={intakes} lessons={lessons} enrolled={enrolled} data={data} busy={busy} run={run} openDetail={openDetail} /></main><DetailTabs active={tab} onChange={setTab} tabs={[{ id: "summary", label: "Summary" }, { id: "intakes", label: "Intakes", count: intakes.length }, { id: "settings", label: "Settings" }]} onClose={close} /></div></aside></div>;
}

function CourseCatalogueContent({ tab, course, intakes, lessons, enrolled, data, busy, run, openDetail }: { tab: string; course: Row; intakes: Row[]; lessons: Row[]; enrolled: Row[]; data: PortalData; busy: boolean; run: (action: string, values?: Row) => Promise<void>; openDetail: (detail: Exclude<Detail, null>) => void }) {
  if (tab === "intakes") return <CourseIntakeLibrary course={course} intakes={intakes} data={data} busy={busy} run={run} openDetail={openDetail} />;
  ...
  return <><section className="detail-metrics"><DetailMetric label="Intakes" value={String(intakes.length)} /><DetailMetric label="In progress" value={String(active)} /><DetailMetric label="Upcoming" value={String(upcoming)} /></section><section className="sheet-section course-sku-overview"><div className="sheet-section-title"><h3>Course SKU</h3><Status value={get(course, "status")} /></div><div className="current-lesson-card" style={eventStyle(course)}><CourseVisual course={course} /><div><strong>{get(course, "default_sessions")} lessons per intake</strong><p>{get(course, "default_minutes")} minutes each · Standard fee {amount(course.list_price)}</p><span><UsersRound size={14} />{enrolled.length} learner bookings across all intakes</span></div></div></section><CourseIntakeCards intakes={intakes} sessions={data.sessions} openDetail={openDetail} /></>;
}

function CourseIntakeLibrary({ course, intakes, data, busy, run, openDetail }: { course: Row; intakes: Row[]; data: PortalData; busy: boolean; run: (action: string, values?: Row) => Promise<void>; openDetail: (detail: Exclude<Detail, null>) => void }) {
  const [adding, setAdding] = useState(false);
  function createIntake(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); void run("createClassRun", { ...values, courseId: get(course, "id") }).then(() => setAdding(false)); }
  return <><section className="sheet-section"><div className="sheet-section-title"><h3>Course intakes</h3><button className="primary-button" type="button" onClick={() => setAdding((value) => !value)}><Plus size={15} />New intake</button></div>{adding ? <form className="detail-edit-form class-open-form" onSubmit={createIntake}><label className="form-field"><span>Term</span><select name="termId" required defaultValue={get(data.terms[0], "id")}>{data.terms.map((term) => <option key={get(term, "id")} value={get(term, "id")}>{get(term, "name")}</option>)}</select></label><FormField name="name" label="Intake name" defaultValue={get(course, "title") + " · New intake"} required /><FormField name="capacity" type="number" label="Capacity" defaultValue="16" min="1" required /><FormField name="price" type="number" label="Course fee (RM)" defaultValue={get(course, "list_price")} min="0" required /><div className="detail-form-actions"><button className="quiet-button" type="button" onClick={() => setAdding(false)}>Cancel</button><button className="primary-button" disabled={busy} type="submit"><Check size={16} />Create intake</button></div></form> : null}</section><CourseIntakeCards intakes={intakes} sessions={data.sessions} openDetail={openDetail} /></>;
}
...
function DetailTabIcon({ id }: { id: string }) { const Icon = id === "summary" ? Home : id === "modify" ? Settings2 : id === "courses" || id === "lessons" || id === "intakes" ? BookOpen : id === "students" ? Users : id === "payments" || id === "income" || id === "finance" ? Banknote : id === "communication" ? Mail : CalendarDays; return <Icon size={18} />; }
