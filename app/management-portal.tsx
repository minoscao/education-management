...
function CourseRunLibrary({ data, run, busy, t, onOpen }: { data: PortalData; run: (action: string, values?: Row) => Promise<void>; busy: boolean; t: typeof copy.en; onOpen: (id: string) => void }) {
  ...
  return <section className="operation-stack"><div className="view-intro"><div><h2>Courses</h2><p>Choose a course, then select its named class intake to plan lessons, teachers and learners.</p></div><button className="primary-button" type="button" onClick={() => setAdding(!adding)}><Plus size={16} />Open class</button></div>{adding ? <section className="management-panel"><h3>Open class</h3><form className="inline-form" onSubmit={createRun}><SelectField name="courseId" label="Course product" rows={data.courses} value="" /><SelectField name="termId" label={t.term} rows={data.terms} value="" /><FormField name="name" label="Class name" placeholder="e.g. July 2026 · Saturday AM" required /><FormField name="capacity" type="number" label={t.capacity} defaultValue="16" min="1" required /><FormField name="price" type="number" label={`${t.price} (RM)`} defaultValue="0" min="0" /><button className="primary-button" disabled={busy} type="submit"><Plus size={16} />Open class</button></form></section> : null}<div className="run-card-grid">{courseGroups.map(({ course, intakes }) => { const selected = intakes.find((item) => get(item, "id") === selectedRuns[get(course, "id")]) ?? intakes.find((item) => get(item, "status") !== "finished") ?? intakes[0]; return <article className="run-card run-course-group" key={get(course, "id")}><button type="button" className="run-card-open" onClick={() => onOpen(get(selected, "id"))}><CourseVisual course={course} /><div className="run-card-body"><span className="code">{get(course, "code")}</span><h3>{get(course, "title")}<span className="class-name-chip compact" style={eventStyle(selected)}>{get(selected, "name")}</span></h3><p>{get(selected, "term_name")}</p><div className="run-card-facts"><span><UsersRound size={15} />{get(selected, "student_count")}/{get(selected, "capacity")}</span><span><CalendarDays size={15} />{get(selected, "session_count")} lessons</span></div><footer><Status value={cohortPhase(selected, data.sessions.filter((session) => get(session, "class_run_id") === get(selected, "id")))} /><span>{cohortTiming(selected, data.sessions.filter((session) => get(session, "class_run_id") === get(selected, "id")))}</span></footer></div><ChevronRight className="run-card-arrow" size={20} /></button><ClassIntakeTabs intakes={intakes} activeId={get(selected, "id")} onChange={(id) => setSelectedRuns((value) => ({ ...value, [get(course, "id")]: id }))} /></article>; })}{!courseGroups.length ? <Empty text={t.noData} /> : null}</div></section>;
}
