...
type Detail = { kind: "session" | "student" | "teacher" | "room" | "course" | "courseCatalog" | "studentCreate"; id: string; startsAt?: string; edit?: boolean } | null;

type PortalData = {
  terms: Row[]; courses: Row[]; runs: Row[]; sessions: Row[]; students: Row[]; teachers: Row[]; campuses: Row[]; classrooms: Row[];
  enrollments: Row[]; invoices: Row[]; payments: Row[]; attendance: Row[]; messages: Row[]; notifications: Row[]; resourceBookings: Row[]; teacherBookings: Row[]; conflicts: Row[];
  settings: { businessHours: { start: string; end: string; source: "configured" | "bookings" }; mail: { sender: string; inboundProtocol: string; inboundHost: string; inboundPort: string; smtpHost: string; smtpPort: string } };
...
const emptyData: PortalData = {
  terms: [], courses: [], runs: [], sessions: [], students: [], teachers: [], campuses: [], classrooms: [], enrollments: [], invoices: [], payments: [], attendance: [], messages: [], notifications: [], resourceBookings: [], teacherBookings: [], conflicts: [], settings: { businessHours: { start: "08:00", end: "20:00", source: "bookings" }, mail: { sender: "", inboundProtocol: "IMAP", inboundHost: "", inboundPort: "993", smtpHost: "", smtpPort: "587" } },
...
export function ManagementPortal() {
  ...
  return <main className={`operation-app role-${role} student-theme-${studentTheme}${view === "calendar" ? " calendar-screen" : ""}${sidebarCollapsed ? " sidebar-collapsed" : ""}`}>
    <aside className="operation-sidebar">
      ...
      {role === "admin" ? <><NavGroup label="DAILY OPERATIONS" current={view} setView={setView} items={[["dashboard", Home, "Dashboard"], ["calendar", CalendarDays, "Schedule"], ["classes", BookOpen, "Courses"], ["campus", MapIcon, "Campus live"], ["students", Users, "Students"], ["payments", Banknote, "Payments"], ["reports", Settings2, "Reports"]]} /><NavGroup label="SETUP" current={view} setView={setView} items={[["courses", LayoutGrid, "Course catalogue"], ["teachers", UserRound, "Teacher setup"], ["classrooms", DoorOpen, "Campus setup"], ["settings", SlidersHorizontal, "Business rules"]]} /></> : null}
    ...
    <section className="operation-workspace">
      ...
      {view === "studentHome" ? <StudentHome data={data} studentId={selectedStudentId} onOpen={(id) => setDetail({ kind: "session", id })} /> : null}
      {view === "studentCourses" ? <StudentCourses data={data} studentId={selectedStudentId} /> : null}
...
function pageTitle(view: View, t: typeof copy.en) {
  const titles: Record<View, string> = { dashboard: "Dashboard", calendar: "Schedule", campus: "Campus live", students: t.students, teachers: "Teacher setup", courses: "Course catalogue", classrooms: "Campus setup", classes: "Courses", enrollment: "Enrolments", payments: "Payments", reports: t.reports, settings: "Business rules", teacherHome: "Today", studentHome: "My learning", studentCourses: "My courses", studentCalendar: "My timetable" };
  return titles[view];
}
...
function StudentHome({ data, studentId, onOpen }: { data: PortalData; studentId: string; onOpen: (id: string) => void }) {
  ...
  return <section className="student-portal"><section className="student-hero"><img src="/assets/student/learning-hero.png" alt="" /><div className="student-hero-copy"><div className="student-profile"><Avatar person={student} /><span>{get(student, "level")} learner</span></div><p>Good to see you</p><h2>{get(student, "name").split(" ")[0]}!</h2><strong>Small steps make big progress.</strong></div><div className="student-stars"><Sparkles size={20} /><b>{attended}</b><span>learning wins</span></div></section><div className="student-focus-grid student-focus-grid-compact"><article className="student-next"><span>UP NEXT</span><h3>{get(next, "course_title") || "Your next lesson"}</h3><p>{get(next, "topic") || "A new learning adventure"}</p><div><Clock3 size={16} /><strong>{next ? `${timePart(next.starts_at)} · ${datePart(next.starts_at)}` : "No lesson booked"}</strong></div>{next ? <button type="button" onClick={() => onOpen(get(next, "id"))}>View lesson <ChevronRight size={16} /></button> : null}</article><article className="student-reminder"><ClipboardCheck size={22} /><div><span>THIS WEEK</span><strong>{sessions.slice(0, 2).length} lessons ahead</strong><p>Pack your notebook and arrive ready.</p></div></article></div><section className="student-learning-section"><div className="student-section-title"><div><span>MY LEARNING</span><h3>Keep going, you are doing great</h3></div><b>{enrollments.length} courses</b></div><div className="course-card-gallery">{enrollments.map((enrollment) => { const runSessions = sessions.filter((item) => get(item, "class_run_id") === get(enrollment, "class_run_id")); const progress = runSessions.length ? Math.min(100, Math.round((data.attendance.filter((item) => get(item, "student_id") === get(student, "id") && runSessions.some((session) => get(session, "id") === get(item, "class_session_id")) && ["present", "late"].includes(get(item, "status"))).length / runSessions.length) * 100)) : 0; return <article className="student-course-card" key={get(enrollment, "id")} style={eventStyle({ course_color: get(enrollment, "course_color") })}><CourseVisual course={{ title: get(enrollment, "course_title"), subject: get(enrollment, "course_title"), display_color: get(enrollment, "course_color") }} /><div><h4>{get(enrollment, "course_title")}</h4><p>{runSessions.length} lessons planned</p><div className="mini-progress"><i style={{ width: `${progress}%` }} /></div><small>{progress}% complete</small></div><strong className="course-progress-number">{progress}%</strong></article>; })}</div></section></section>;
}

function StudentCourses({ data, studentId }: { data: PortalData; studentId: string }) { const { student, enrollments, sessions } = studentLearning(data, studentId); return <section className="student-portal"><section className="student-simple-heading"><Sparkles size={22} /><div><span>MY COURSES</span><h2>Everything you are learning</h2></div></section><div className="course-card-gallery">{enrollments.map((enrollment) => { const runSessions = sessions.filter((item) => get(item, "class_run_id") === get(enrollment, "class_run_id")); const attended = data.attendance.filter((item) => get(item, "student_id") === get(student, "id") && runSessions.some((session) => get(session, "id") === get(item, "class_session_id")) && ["present", "late"].includes(get(item, "status"))).length; const progress = runSessions.length ? Math.round(attended / runSessions.length * 100) : 0; return <article className="student-course-card" key={get(enrollment, "id")} style={eventStyle({ course_color: get(enrollment, "course_color") })}><CourseVisual course={{ title: get(enrollment, "course_title"), subject: get(enrollment, "course_title"), display_color: get(enrollment, "course_color") }} /><div><h4>{get(enrollment, "course_title")}</h4><p>{get(enrollment, "run_name")}</p><span>{attended}/{runSessions.length} attended</span></div><strong className="course-progress-number">{progress}%</strong></article>; })}</div></section>; }
...
function TimelineEvent({ event, window, onOpen, draggable = false }: { event: Row; window: ScheduleWindow; onOpen: (id: string) => void; draggable?: boolean }) {
  ...
  return <button type="button" draggable={draggable} className={`timeline-event${draggable ? " draggable-lesson" : ""}`} style={{ ...eventStyle(event), top: `${top}%`, height: `${height}%` }} onDragStart={(dragEvent) => dragEvent.dataTransfer.setData("application/x-lesson", get(event, "id"))} onClick={() => onOpen(eventSessionId(event))}><span>{timePart(event.starts_at)}</span><strong>{get(event, "course_title")}</strong><em>{get(event, "topic") || get(event, "classroom_name") || get(event, "student_name")}</em></button>;
}
  ...
  return <section className="operation-stack">
    ...
    <div className="course-grid">
      {data.courses.map((course) => <button type="button" key={get(course, "id")} className="course-card course-card-button" onClick={() => onOpen(get(course, "id"))}>
        ...
        <div className="course-body"><span className="code">{get(course, "code")}</span><h3>{get(course, "title")}</h3><p>{get(course, "subject")} · {get(course, "level")}</p><div className="course-facts"><span><CalendarDays size={15} />{get(course, "default_sessions")} lessons</span><span><Banknote size={15} />{amount(course.list_price)}</span></div><footer><Status value={get(course, "status")} /><span>{get(course, "run_count")} classes</span></footer></div>
...
function CourseRunLibrary({ data, run, busy, t, onOpen }: { data: PortalData; run: (action: string, values?: Row) => Promise<void>; busy: boolean; t: typeof copy.en; onOpen: (id: string) => void }) {
  ...
  return <section className="operation-stack"><div className="view-intro"><div><h2>Courses</h2><p>Open a course card to plan its lessons, teaching team and student list.</p></div><button className="primary-button" type="button" onClick={() => setAdding(!adding)}><Plus size={16} />Open class</button></div>{adding ? <section className="management-panel"><h3>Open class</h3><form className="inline-form" onSubmit={createRun}><SelectField name="courseId" label="Course product" rows={data.courses} value="" /><SelectField name="termId" label={t.term} rows={data.terms} value="" /><FormField name="name" label="Class name" required /><FormField name="capacity" type="number" label={t.capacity} defaultValue="16" min="1" required /><FormField name="price" type="number" label={`${t.price} (RM)`} defaultValue="0" min="0" /><button className="primary-button" disabled={busy} type="submit"><Plus size={16} />Open class</button></form></section> : null}<div className="run-card-grid">{data.runs.map((item) => <button type="button" key={get(item, "id")} className="run-card" onClick={() => onOpen(get(item, "id"))}><CourseVisual course={item} /><div className="run-card-body"><span className="code">{get(item, "code")}</span><h3>{get(item, "course_title")}</h3><p>{get(item, "name")}</p><div className="run-card-facts"><span><UsersRound size={15} />{get(item, "student_count")}/{get(item, "capacity")}</span><span><CalendarDays size={15} />{get(item, "session_count")} lessons</span></div><footer><Status value={get(item, "status")} /><span>{get(item, "term_name")}</span></footer></div><ChevronRight className="run-card-arrow" size={20} /></button>)}{!data.runs.length ? <Empty text={t.noData} /> : null}</div></section>;
}
...
function CourseRunEditor({ runItem, data, run, busy, t, onBack, onOpen }: { runItem: Row; data: PortalData; run: (action: string, values?: Row) => Promise<void>; busy: boolean; t: typeof copy.en; onBack: () => void; onOpen: (id: string) => void }) {
  const [addingLesson, setAddingLesson] = useState(false);
  const [tab, setTab] = useState(detail.edit ? "modify" : "summary");
...
function CourseCatalogueDrawer({ detail, data, busy, run, close, closeAll, openDetail }: { detail: Exclude<Detail, null>; data: PortalData; busy: boolean; run: (action: string, values?: Row) => Promise<void>; close: () => void; closeAll: () => void; openDetail: (detail: Exclude<Detail, null>) => void }) {
  ...
  const classes = data.runs.filter((item) => get(item, "course_id") === detail.id);
  const lessons = data.sessions.filter((item) => classes.some((classRun) => get(classRun, "id") === get(item, "class_run_id")));
  const enrolled = data.enrollments.filter((item) => classes.some((classRun) => get(classRun, "id") === get(item, "class_run_id")) && get(item, "status") === "enrolled");
  return <div className="sheet-backdrop" role="presentation" onMouseDown={closeAll}>
    <aside className="detail-sheet detail-course" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
      <header className="entity-header"><div className="entity-heading"><div className="entity-icon"><BookOpen size={22} /></div><div><span className="sheet-eyebrow">Course</span><h2>{get(course, "title")}</h2><p>{get(course, "subject")} · {get(course, "level")}</p></div></div><div className="entity-header-actions"><button className="quiet-button" type="button" onClick={() => setTab("settings")}>Edit course</button></div></header>
      <div className="entity-workspace detail-workspace"><main className="entity-main"><CourseCatalogueContent tab={tab} course={course} classes={classes} lessons={lessons} enrolled={enrolled} data={data} busy={busy} run={run} openDetail={openDetail} /></main><DetailTabs active={tab} onChange={setTab} tabs={[{ id: "summary", label: "Summary" }, { id: "classes", label: "Classes", count: classes.length }, { id: "settings", label: "Settings" }]} onClose={close} /></div>
    </aside>
  </div>;
}

function CourseCatalogueContent({ tab, course, classes, lessons, enrolled, data, busy, run, openDetail }: { tab: string; course: Row; classes: Row[]; lessons: Row[]; enrolled: Row[]; data: PortalData; busy: boolean; run: (action: string, values?: Row) => Promise<void>; openDetail: (detail: Exclude<Detail, null>) => void }) {
  if (tab === "classes") return <CourseClassLibrary course={course} classes={classes} data={data} busy={busy} run={run} openDetail={openDetail} />;
  if (tab === "settings") return <CourseProductSettings course={course} busy={busy} run={run} />;
  return <><section className="detail-metrics"><DetailMetric label="Classes" value={String(classes.length)} /><DetailMetric label="Planned lessons" value={String(lessons.length)} /><DetailMetric label="Enrolled students" value={String(enrolled.length)} /></section><section className="sheet-section"><div className="sheet-section-title"><h3>Course overview</h3><Status value={get(course, "status")} /></div><div className="current-lesson-card" style={eventStyle(course)}><CourseVisual course={course} /><div><strong>{get(course, "default_sessions")} lessons per class</strong><p>{get(course, "default_minutes")} minutes each · Standard fee {amount(course.list_price)}</p><span><UsersRound size={14} />{classes.length} classes currently open</span></div></div></section><CourseClassCards classes={classes} sessions={data.sessions} openDetail={openDetail} limit={3} /></>;
}

function CourseClassLibrary({ course, classes, data, busy, run, openDetail }: { course: Row; classes: Row[]; data: PortalData; busy: boolean; run: (action: string, values?: Row) => Promise<void>; openDetail: (detail: Exclude<Detail, null>) => void }) {
  const [adding, setAdding] = useState(false);
  function createClass(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); void run("createClassRun", { ...values, courseId: get(course, "id") }).then(() => setAdding(false)); }
  return <><section className="sheet-section"><div className="sheet-section-title"><h3>Classes</h3><button className="primary-button" type="button" onClick={() => setAdding((value) => !value)}><Plus size={15} />Open class</button></div>{adding ? <form className="detail-edit-form class-open-form" onSubmit={createClass}><label className="form-field"><span>Term</span><select name="termId" required defaultValue={get(data.terms[0], "id")}>{data.terms.map((term) => <option key={get(term, "id")} value={get(term, "id")}>{get(term, "name")}</option>)}</select></label><FormField name="name" label="Class name" defaultValue={`${get(course, "title")} class`} required /><FormField name="capacity" type="number" label="Capacity" defaultValue="16" min="1" required /><FormField name="price" type="number" label="Course fee (RM)" defaultValue={get(course, "list_price")} min="0" required /><div className="detail-form-actions"><button className="quiet-button" type="button" onClick={() => setAdding(false)}>Cancel</button><button className="primary-button" disabled={busy} type="submit"><Check size={16} />Create class</button></div></form> : null}</section><CourseClassCards classes={classes} sessions={data.sessions} openDetail={openDetail} /></>;
}

function CourseClassCards({ classes, sessions, openDetail, limit }: { classes: Row[]; sessions: Row[]; openDetail: (detail: Exclude<Detail, null>) => void; limit?: number }) {
  const visible = limit ? classes.slice(0, limit) : classes;
  return <section className="sheet-section course-class-list"><div className="sheet-section-title"><h3>{limit ? "Current classes" : "Configure a class"}</h3><span>{classes.length}</span></div>{visible.map((classRun) => { const classLessons = sessions.filter((session) => get(session, "class_run_id") === get(classRun, "id")); const first = classLessons[0]; return <button type="button" key={get(classRun, "id")} className="course-class-row interactive-list-row" style={eventStyle(first ?? classRun)} onClick={() => openDetail({ kind: "course", id: get(classRun, "id") })}><CourseVisual course={classRun} /><div><strong>{get(classRun, "name")}</strong><p>{first ? `${get(first, "teacher_name")} · ${get(first, "classroom_name")}` : "Teacher and classroom to be configured"}</p><span><CalendarDays size={14} />{first ? `${datePart(first.starts_at)} · ${timeRange(first)}` : "No lessons scheduled"}</span></div><ChevronRight size={18} /></button>; })}{!classes.length ? <Empty text="Open a class to add its teaching schedule." /> : null}</section>;
}
...
function CourseProductSettings({ course, busy, run }: { course: Row; busy: boolean; run: (action: string, values?: Row) => Promise<void> }) {
  ...
  return <section className="sheet-section"><div className="sheet-section-title"><h3>Course settings</h3><span>Product</span></div><form className="detail-edit-form" onSubmit={save}><FormField name="title" label="Course name" defaultValue={get(course, "title")} required /><FormField name="subject" label="Subject" defaultValue={get(course, "subject")} required /><FormField name="level" label="Level" defaultValue={get(course, "level")} required /><FormField name="sessions" type="number" label="Lessons per class" defaultValue={get(course, "default_sessions")} min="1" required /><FormField name="minutes" type="number" label="Minutes per lesson" defaultValue={get(course, "default_minutes")} min="30" required /><FormField name="price" type="number" label="Standard fee (RM)" defaultValue={get(course, "list_price")} min="0" step="0.01" required /><CourseColourPicker value={colour} onChange={setColour} disabled={busy} /><div className="detail-form-actions"><button className="primary-button" disabled={busy} type="submit"><Check size={16} />Save course</button></div></form></section>;
}
...
function DetailSheet({ detail, data, t, busy, run, close, closeAll, openDetail }: { detail: Exclude<Detail, null>; data: PortalData; t: typeof copy.en; busy: boolean; run: (action: string, values?: Row) => Promise<void>; close: () => void; closeAll: () => void; openDetail: (detail: Exclude<Detail, null>) => void }) {
  ...
  const item = detail.kind === "session" ? data.sessions.find((row) => get(row, "id") === detail.id) : detail.kind === "student" ? data.students.find((row) => get(row, "id") === detail.id) : detail.kind === "teacher" ? data.teachers.find((row) => get(row, "id") === detail.id) : detail.kind === "course" ? data.runs.find((row) => get(row, "id") === detail.id) : data.classrooms.find((row) => get(row, "id") === detail.id);
  const [tab, setTab] = useState(detail.edit && detail.kind === "session" ? "modify" : "summary");
  ...
  const tabs = detail.kind === "student" ? [{ id: "summary", label: "Summary" }, { id: "courses", label: "Courses" }, { id: "payments", label: "Payments", count: studentInvoices.length }, { id: "communication", label: "Email", count: studentMessages.length }] : detail.kind === "teacher" ? [{ id: "summary", label: "Summary" }, { id: "courses", label: "Courses", count: teacherLessons.length }, { id: "income", label: "Income" }] : detail.kind === "session" ? [{ id: "summary", label: "Summary" }, { id: "lessons", label: "Lessons", count: courseSessions.length }, { id: "modify", label: "Modify" }] : detail.kind === "course" ? [{ id: "summary", label: "Summary" }, { id: "lessons", label: "Lessons", count: runLessons.length }, { id: "students", label: "Students", count: runStudents.length }, { id: "configure", label: "Configure" }] : [{ id: "summary", label: "Summary" }, { id: "schedule", label: "Schedule", count: roomLessons.length }];
...
function DetailTabs({ active, onChange, tabs, onClose }: { active: string; onChange: (value: string) => void; tabs: { id: string; label: string; count?: number }[]; onClose: () => void }) { return <nav className="detail-tabs-right" aria-label="Details sections"><button className="drawer-close-tab" type="button" onClick={onClose} title="Close details" aria-label="Close details"><ChevronRight size={20} /></button>{tabs.map((tab) => <button type="button" className={active === tab.id ? "active" : ""} onClick={() => onChange(tab.id)} key={tab.id}><DetailTabIcon id={tab.id} /><span>{tab.label}</span>{tab.count !== undefined ? <b>{tab.count}</b> : null}</button>)}</nav>; }
function DetailTabIcon({ id }: { id: string }) { const Icon = id === "summary" ? Home : id === "modify" ? Settings2 : id === "courses" || id === "lessons" ? BookOpen : id === "students" ? Users : id === "payments" || id === "income" || id === "finance" ? Banknote : id === "communication" ? Mail : CalendarDays; return <Icon size={18} />; }
...
function CourseDetailContent({ tab, item, lessons, students, data, busy, run, openDetail }: { tab: string; item: Row; lessons: Row[]; students: Row[]; data: PortalData; busy: boolean; run: (action: string, values?: Row) => Promise<void>; openDetail: (detail: Exclude<Detail, null>) => void }) {
  ...
  const activeLessons = lessons.filter((lesson) => asTime(lesson.ends_at) >= now);
  const seats = Math.max(0, Number(get(item, "capacity")) - students.length);
  if (tab === "configure") return <ClassRunScheduleForm runItem={item} lessons={lessons} data={data} busy={busy} run={run} openDetail={openDetail} />;
  if (tab === "lessons") return <><LessonSequence title="In progress & upcoming" sessions={activeLessons} currentId="" expanded onOpen={(session) => openDetail({ kind: "session", id: get(session, "id") })} /><LessonSequence title="Completed lessons" sessions={completedLessons} currentId="" expanded onOpen={(session) => openDetail({ kind: "session", id: get(session, "id") })} /></>;
  ...
  return <><section className="detail-metrics"><DetailMetric label="Enrolled" value={`${students.length}/${get(item, "capacity")}`} /><DetailMetric label="Lessons" value={String(lessons.length)} /><DetailMetric label="Course fee" value={amount(item.price)} /></section><section className="sheet-section"><h3>Class overview</h3><div className="current-lesson-card" style={eventStyle(item)}><CourseVisual course={item} /><div><strong>{get(item, "name")}</strong><p>{get(item, "term_name")}</p><span><UsersRound size={14} />{students.length} students enrolled</span></div></div></section><LessonSequence title="In progress & upcoming" sessions={activeLessons} currentId="" onOpen={(session) => openDetail({ kind: "session", id: get(session, "id") })} /></>;
}

function ClassRunScheduleForm({ runItem, lessons, data, busy, run, openDetail }: { runItem: Row; lessons: Row[]; data: PortalData; busy: boolean; run: (action: string, values?: Row) => Promise<void>; openDetail: (detail: Exclude<Detail, null>) => void }) {
  const firstLesson = lessons[0];
  const teacherBooking = data.teacherBookings.find((booking) => get(booking, "class_session_id") === get(firstLesson, "id"));
  const roomBooking = data.resourceBookings.find((booking) => get(booking, "class_session_id") === get(firstLesson, "id"));
  const duration = firstLesson ? Math.max(30, minutesOfDay(firstLesson.ends_at) - minutesOfDay(firstLesson.starts_at)) : 90;
  function applyToClass(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); void run("configureClassRun", { ...values, runId: get(runItem, "id") }); }
  return <><section className="detail-metrics"><DetailMetric label="Lessons" value={String(lessons.length)} /><DetailMetric label="Teacher" value={get(teacherBooking, "teacher_name") || "Not set"} /><DetailMetric label="Room" value={get(roomBooking, "classroom_name") || "Not set"} /></section><section className="sheet-section class-schedule-config"><div className="sheet-section-title"><h3>Apply class schedule</h3><span>All lessons</span></div><p>Choose the weekly teacher, classroom and start time. This updates every lesson in this class and checks rooms, teachers and enrolled students before saving.</p><form className="detail-edit-form" onSubmit={applyToClass}><label className="form-field"><span>Teacher</span><select name="teacherId" defaultValue={get(teacherBooking, "teacher_id")} required>{data.teachers.map((teacher) => <option key={get(teacher, "id")} value={get(teacher, "id")}>{get(teacher, "name")} · {get(teacher, "subject")}</option>)}</select></label><label className="form-field"><span>Classroom</span><select name="classroomId" defaultValue={get(roomBooking, "classroom_id")} required>{data.classrooms.map((room) => <option key={get(room, "id")} value={get(room, "id")}>{get(room, "name")} · {get(room, "capacity")} seats</option>)}</select></label><label className="form-field"><span>First lesson date</span><input name="startDate" type="date" defaultValue={firstLesson ? datePart(firstLesson.starts_at) : ""} required /></label><label className="form-field"><span>Weekly start time</span><input name="startTime" type="time" defaultValue={firstLesson ? timePart(firstLesson.starts_at) : "09:00"} required /></label><FormField name="durationMinutes" type="number" label="Lesson duration (minutes)" defaultValue={String(duration)} min="30" step="15" required /><div className="detail-form-actions"><button className="primary-button" disabled={busy || !lessons.length} type="submit"><CalendarDays size={16} />Apply to all lessons</button></div></form>{!lessons.length ? <p className="form-error">Add at least one lesson before applying a class schedule.</p> : null}</section><section className="sheet-section"><div className="sheet-section-title"><h3>Individual lessons</h3><span>{lessons.length}</span></div><div className="class-individual-lessons">{lessons.map((lesson) => <button type="button" key={get(lesson, "id")} className="course-class-row interactive-list-row" style={eventStyle(lesson)} onClick={() => openDetail({ kind: "session", id: get(lesson, "id"), edit: true })}><span className="lesson-number">{get(lesson, "session_no")}</span><div><strong>{get(lesson, "topic")}</strong><p>{get(lesson, "teacher_name")} · {get(lesson, "classroom_name")}</p><span><Clock3 size={14} />{datePart(lesson.starts_at)} · {timeRange(lesson)}</span></div><Settings2 size={17} /></button>)}</div></section></>;
}
...
function PaymentLedger({ invoices, payments, run, busy }: { invoices: Row[]; payments: Row[]; run: (action: string, values?: Row) => Promise<void>; busy: boolean }) {
...
function SessionDetailContent({ tab, item, courseSessions, roster, students, classrooms, teachers, draftStartsAt, enrollStudentId, setEnrollStudentId, busy, run, t, onSaved, openDetail }: { tab: string; item: Row; courseSessions: Row[]; roster: Row[]; students: Row[]; classrooms: Row[]; teachers: Row[]; draftStartsAt?: string; enrollStudentId: string; setEnrollStudentId: (value: string) => void; busy: boolean; run: (action: string, values?: Row) => Promise<void>; t: typeof copy.en; onSaved: () => void; openDetail: (detail: Exclude<Detail, null>) => void }) {
  ...
  if (tab === "modify") return <SessionModifyForm item={item} classrooms={classrooms} teachers={teachers} draftStartsAt={draftStartsAt} busy={busy} run={run} onSaved={onSaved} />;
...
function SessionModifyForm({ item, classrooms, teachers, draftStartsAt, busy, run, onSaved }: { item: Row; classrooms: Row[]; teachers: Row[]; draftStartsAt?: string; busy: boolean; run: (action: string, values?: Row) => Promise<void>; onSaved: () => void }) {
  const [error, setError] = useState("");
  const room = classrooms.find((entry) => get(entry, "name") === get(item, "classroom_name"));
  const teacher = teachers.find((entry) => get(entry, "name") === get(item, "teacher_name"));
  const startDefault = draftStartsAt || get(item, "starts_at").replace(" ", "T");
  const duration = Math.max(15, minutesOfDay(item.ends_at) - minutesOfDay(item.starts_at));
  const endDefault = draftStartsAt ? shiftDateTime(draftStartsAt, duration) : get(item, "ends_at").replace(" ", "T");
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setError("");
    try {
      const values = Object.fromEntries(new FormData(event.currentTarget));
      await run("updateSession", { ...values, sessionId: get(item, "id") });
      onSaved();
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Lesson could not be updated.");
    }
  }
  return <><section className="detail-metrics"><DetailMetric label="Current time" value={`${datePart(item.starts_at)} ${timeRange(item)}`} /><DetailMetric label="Current room" value={get(item, "classroom_name")} /></section><section className="sheet-section session-modify"><div className="sheet-section-title"><h3>Modify lesson</h3><span>Checks availability</span></div><p>Save to update this lesson for the class, teacher and enrolled students.</p><form className="detail-edit-form" onSubmit={submit}><FormField name="topic" label="Lesson topic" defaultValue={get(item, "topic")} required /><label className="form-field"><span>Date & start time</span><input name="startsAt" type="datetime-local" defaultValue={startDefault} required /></label><label className="form-field"><span>End time</span><input name="endsAt" type="datetime-local" defaultValue={endDefault} required /></label><label className="form-field"><span>Classroom</span><select name="classroomId" defaultValue={get(room, "id")} required>{classrooms.map((entry) => <option key={get(entry, "id")} value={get(entry, "id")}>{get(entry, "name")} - {get(entry, "capacity")} seats</option>)}</select></label><label className="form-field"><span>Teacher</span><select name="teacherId" defaultValue={get(teacher, "id")} required>{teachers.map((entry) => <option key={get(entry, "id")} value={get(entry, "id")}>{get(entry, "name")} - {get(entry, "subject")}</option>)}</select></label>{error ? <p className="form-error">{error}</p> : null}<div className="detail-form-actions"><button className="primary-button" disabled={busy} type="submit"><Check size={16} />Save lesson changes</button></div></form></section></>;
}
...
function InvoiceTable({ rows, run, busy }: { rows: Row[]; run: (action: string, values?: Row) => Promise<void>; busy: boolean }) {
  const [invoiceToPay, setInvoiceToPay] = useState<Row | null>(null);
  const columns: GridColumn[] = [{ key: "invoice_no", label: "Invoice" }, { key: "student_name", label: "Student" }, { key: "course_title", label: "Course" }, { key: "total_amount", label: "Due" }, { key: "paid_amount", label: "Paid" }, { key: "balance", label: "Balance" }, { key: "status", label: "Status" }, { key: "actions", label: "Actions" }];
