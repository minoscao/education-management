...
const emptyData: PortalData = {
  terms: [], courses: [], runs: [], sessions: [], students: [], teachers: [], campuses: [], classrooms: [], enrollments: [], invoices: [], payments: [], attendance: [], resourceBookings: [], teacherBookings: [], conflicts: [], settings: { businessHours: { start: "08:00", end: "20:00", source: "bookings" } },
...
export function ManagementPortal() {
  ...
  return <main className={`operation-app${sidebarCollapsed ? " sidebar-collapsed" : ""}`}>
    ...
    {detail ? <DetailSheet key={`${detail.kind}-${detail.id}`} detail={detail} data={data} t={t} busy={busy} run={run} close={() => setDetail(null)} /> : null}
  </main>;
}
...
function CourseVisual({ course }: { course: Row }) { const subject = get(course, "subject").toLowerCase(); const Icon = subject.includes("music") || subject.includes("violin") ? Music2 : subject.includes("math") ? LayoutGrid : BookOpen; return <div className="course-visual" style={{ "--course-colour": eventColour({ course_color: get(course, "display_color") || get(course, "run_course_color") || get(course, "course_color") }) } as React.CSSProperties}><img src={courseCover({ ...course, title: get(course, "title") || get(course, "course_title") })} alt="" /><div><Icon size={22} /><span>{get(course, "subject").slice(0, 2).toUpperCase()}</span></div></div>; }
...
function DetailSheet({ detail, data, t, busy, run, close }: { detail: Exclude<Detail, null>; data: PortalData; t: typeof copy.en; busy: boolean; run: (action: string, values?: Row) => Promise<void>; close: () => void }) {
  ...
  const title = detail.kind === "session" ? get(item, "course_title") : get(item, "name");
  const subtitle = detail.kind === "session" ? `${get(item, "topic")} · ${get(item, "starts_at")}` : detail.kind === "student" ? get(item, "level") : detail.kind === "teacher" ? get(item, "subject") : `${get(item, "location")} · ${get(item, "capacity")} seats`;
  const tabs = detail.kind === "student" ? [{ id: "summary", label: "Summary" }, { id: "courses", label: "Courses", count: studentEnrollments.length }, { id: "payments", label: "Payments", count: studentInvoices.length }, { id: "attendance", label: "Attendance" }] : detail.kind === "teacher" ? [{ id: "summary", label: "Summary" }, { id: "courses", label: "Courses", count: teacherLessons.length }, { id: "income", label: "Income" }] : detail.kind === "session" ? [{ id: "summary", label: "Summary" }, { id: "students", label: "Students", count: sessionRoster.length }] : [{ id: "summary", label: "Summary" }, { id: "schedule", label: "Schedule", count: roomLessons.length }];
  ...
  const openStudents = () => setTab("students");
  return <div className="sheet-backdrop" role="presentation" onMouseDown={close}><aside className="detail-sheet" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><header className="entity-header"><div className="entity-heading">{detail.kind === "student" || detail.kind === "teacher" ? <img className="entity-avatar" src={avatarUrl(item)} alt="" /> : <div className="entity-icon">{detail.kind === "session" ? <ClipboardCheck size={22} /> : <DoorOpen size={22} />}</div>}<div><span className="sheet-eyebrow">{detail.kind === "session" ? "Lesson" : detail.kind === "student" ? "Student" : detail.kind === "teacher" ? "Teacher" : "Classroom"}</span><h2>{title}</h2><p>{subtitle}</p></div></div><div className="entity-header-actions">{detail.kind === "student" || detail.kind === "teacher" ? <button className="quiet-button" type="button" onClick={() => setEditing(!editing)}>{editing ? "Cancel edit" : "Edit profile"}</button> : null}{detail.kind === "session" ? <button className="primary-button" type="button" onClick={openStudents}>Check in</button> : null}<button className="header-icon" type="button" onClick={close} title={t.cancel}><X size={18} /></button></div></header><div className="entity-workspace detail-workspace"><main className="entity-main">{detail.kind === "student" ? <StudentDetailContent tab={tab} editing={editing} item={item} enrollments={studentEnrollments} invoices={studentInvoices} attendance={studentAttendance} outstanding={outstanding} busy={busy} saveProfile={saveProfile} run={run} /> : null}{detail.kind === "teacher" ? <TeacherDetailContent tab={tab} editing={editing} item={item} lessons={teacherLessons} total={incomeTotal} paid={incomePaid} busy={busy} saveProfile={saveProfile} /> : null}{detail.kind === "session" ? <SessionDetailContent tab={tab} item={item} roster={sessionRoster} students={data.students} enrollStudentId={enrollStudentId} setEnrollStudentId={setEnrollStudentId} busy={busy} run={run} t={t} /> : null}{detail.kind === "room" ? <RoomDetailContent tab={tab} item={item} lessons={roomLessons} /> : null}</main><DetailTabs active={tab} onChange={setTab} tabs={tabs} /></div></aside></div>;
}

function DetailTabs({ active, onChange, tabs }: { active: string; onChange: (value: string) => void; tabs: { id: string; label: string; count?: number }[] }) { return <nav className="detail-tabs-right" aria-label="Details sections">{tabs.map((tab) => <button type="button" className={active === tab.id ? "active" : ""} onClick={() => onChange(tab.id)} key={tab.id}><span>{tab.label}</span>{tab.count !== undefined ? <b>{tab.count}</b> : null}</button>)}</nav>; }
...
function StudentDetailContent({ tab, editing, item, enrollments, invoices, attendance, outstanding, busy, saveProfile, run }: { tab: string; editing: boolean; item: Row; enrollments: Row[]; invoices: Row[]; attendance: Row[]; outstanding: number; busy: boolean; saveProfile: (event: FormEvent<HTMLFormElement>) => void; run: (action: string, values?: Row) => Promise<void> }) {
  if (tab === "courses") return <ListSection title="Course list" rows={enrollments} fields={[["course_title", "Course"], ["run_name", "Class"], ["contracted_fee", "Course fee"], ["invoice_status", "Payment"]]} moneyKeys={["contracted_fee"]} />;
  if (tab === "payments") return <section className="sheet-section"><div className="detail-metrics"><DetailMetric label="Outstanding" value={amount(outstanding)} /><DetailMetric label="Invoices" value={String(invoices.length)} /></div><InvoiceTable rows={invoices} run={run} busy={busy} /></section>;
  if (tab === "attendance") return <ListSection title="Attendance" rows={attendance} fields={[["course_title", "Course"], ["topic", "Lesson"], ["starts_at", "Date"], ["status", "Status"]]} />;
  return <><section className="detail-metrics"><DetailMetric label="Active courses" value={String(enrollments.length)} /><DetailMetric label="Lessons attended" value={String(attendance.filter((row) => get(row, "status") === "present").length)} /><DetailMetric label="Outstanding" value={amount(outstanding)} /></section>{editing ? <section className="sheet-section"><h3>Edit student</h3><form className="detail-edit-form" onSubmit={saveProfile}><FormField name="name" label="Student name" defaultValue={get(item, "name")} required /><FormField name="level" label="Level" defaultValue={get(item, "level")} required /><FormField name="phone" label="Guardian phone" defaultValue={get(item, "guardian_phone")} /><button className="primary-button" disabled={busy} type="submit"><Check size={16} />Save changes</button></form></section> : <section className="sheet-section"><h3>Student profile</h3><div className="sheet-overview"><Info label="Student code" value={get(item, "code")} /><Info label="Level" value={get(item, "level")} /><Info label="Guardian phone" value={get(item, "guardian_phone")} /><Info label="Status" value={get(item, "status")} /></div></section>}<ListSection title="Upcoming courses" rows={enrollments.slice(0, 3)} fields={[["course_title", "Course"], ["run_name", "Class"], ["invoice_status", "Payment"]]} /></>;
}

function TeacherDetailContent({ tab, editing, item, lessons, total, paid, busy, saveProfile }: { tab: string; editing: boolean; item: Row; lessons: Row[]; total: number; paid: number; busy: boolean; saveProfile: (event: FormEvent<HTMLFormElement>) => void }) {
  if (tab === "courses") return <ListSection title="Teaching schedule" rows={lessons} fields={[["course_title", "Course"], ["run_name", "Class"], ["topic", "Lesson"], ["starts_at", "Start"], ["pay_amount", "Pay"]]} moneyKeys={["pay_amount"]} />;
  if (tab === "income") return <section className="sheet-section"><div className="detail-metrics"><DetailMetric label="Scheduled pay" value={amount(total)} /><DetailMetric label="Paid" value={amount(paid)} /><DetailMetric label="To pay" value={amount(total - paid)} /></div><ListSection title="Income by lesson" rows={lessons} fields={[["course_title", "Course"], ["starts_at", "Date"], ["pay_amount", "Amount"], ["pay_status", "Status"]]} moneyKeys={["pay_amount"]} /></section>;
  return <><section className="detail-metrics"><DetailMetric label="Lessons" value={String(lessons.length)} /><DetailMetric label="Scheduled pay" value={amount(total)} /><DetailMetric label="Paid" value={amount(paid)} /></section>{editing ? <section className="sheet-section"><h3>Edit teacher</h3><form className="detail-edit-form" onSubmit={saveProfile}><FormField name="name" label="Teacher name" defaultValue={get(item, "name")} required /><FormField name="subject" label="Subject" defaultValue={get(item, "subject")} required /><FormField name="phone" label="Phone" defaultValue={get(item, "phone")} /><button className="primary-button" disabled={busy} type="submit"><Check size={16} />Save changes</button></form></section> : <section className="sheet-section"><h3>Teacher profile</h3><div className="sheet-overview"><Info label="Teacher code" value={get(item, "code")} /><Info label="Subject" value={get(item, "subject")} /><Info label="Phone" value={get(item, "phone")} /><Info label="Status" value={get(item, "status")} /></div></section>}<ListSection title="Next lessons" rows={lessons.slice(0, 3)} fields={[["course_title", "Course"], ["topic", "Lesson"], ["starts_at", "Start"]]} /></>;
}

function SessionDetailContent({ tab, item, roster, students, enrollStudentId, setEnrollStudentId, busy, run, t }: { tab: string; item: Row; roster: Row[]; students: Row[]; enrollStudentId: string; setEnrollStudentId: (value: string) => void; busy: boolean; run: (action: string, values?: Row) => Promise<void>; t: typeof copy.en }) {
  if (tab === "summary") return <><section className="detail-metrics"><DetailMetric label="Students" value={String(roster.length)} /><DetailMetric label="Teacher" value={get(item, "teacher_name")} /><DetailMetric label="Room" value={get(item, "classroom_name")} /></section><section className="sheet-section"><h3>Lesson plan</h3><div className="sheet-overview"><Info label="Class" value={get(item, "run_name")} /><Info label="Topic" value={get(item, "topic")} /><Info label="Start" value={get(item, "starts_at")} /><Info label="End" value={get(item, "ends_at")} /></div></section></>;
  return <><section className="sheet-section"><div className="sheet-section-title"><h3>Class register</h3><span>{roster.length}</span></div>{roster.map((row) => <div className="sheet-roster" key={get(row, "id")}><div><strong>{get(row, "student_name")}</strong><small>{amount(row.allocated_fee)}</small></div><Status value={get(row, "status")} /><div className="attendance-buttons">{[["present", t.present], ["late", t.late], ["leave", t.leave], ["absent", t.absent]].map(([status, label]) => <button type="button" key={status} disabled={busy} className={get(row, "status") === status ? "selected" : ""} onClick={() => void run("setAttendance", { studentBookingId: get(row, "student_booking_id"), attendanceStatus: status })}>{label}</button>)}</div></div>)}</section><section className="sheet-section"><div className="sheet-section-title"><h3>Add student</h3></div><div className="sheet-enroll"><select value={enrollStudentId} onChange={(event) => setEnrollStudentId(event.target.value)}><option value="">Select student</option>{students.filter((student) => !roster.some((row) => get(row, "student_id") === get(student, "id"))).map((student) => <option key={get(student, "id")} value={get(student, "id")}>{get(student, "name")}</option>)}</select><button className="primary-button" disabled={busy || !enrollStudentId} type="button" onClick={() => void run("enrollStudent", { runId: get(item, "class_run_id"), studentId: enrollStudentId })}><Plus size={15} />Enroll</button></div></section></>;
}
