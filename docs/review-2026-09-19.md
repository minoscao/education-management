...
- Compact student home at `/student`: available balances, enrolled courses and
  grade-matched online lessons for the next seven days. Fixed onsite/online/study
  actions reuse the existing booking and study reservation flows.
- One shared lesson card and attendance-mode switch are used across home and
  weekly booking. Onsite capacity counts onsite reservations only; online is
  unlimited. Unavailable cards are disabled and fully grayscale.
- Ten locally hosted photographic demo portraits and three-sentence teacher
  profiles are seeded once without overwriting existing profiles. These are
  demonstration images, not the institution's actual staff. Administrators can
  replace photos and edit biographies.
- Unbooked same-grade online entry validates the classroom link and time window,
  confirms one credit, then atomically books, consumes and marks attendance.
  Concurrent/repeated entry consumes once. Cancelled legacy attendance is reused.
- Browser accessibility inspection confirmed compact home, balances, course and
  drop-in cards, teacher profiles and all three bottom actions. Screenshot/click
  verification remains blocked by browser-control timeouts; do not treat it as
  a completed visual regression check.
- No real classroom URLs were invented or added to this unauthenticated demo.
  A missing link prevents entry and credit consumption.
- All 41 tests and the production build pass. Local API verification returned
  ten teachers, each with a stored photo and biography.
